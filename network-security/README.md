...
- изучить основные принципы работы сетевых утилит `nmap` и `tshark`
- научиться определять версию ОС сервера с помощью `nmap`
- научиться анализировать сетевой трафик с помощью `tshark`
- научиться использовать систему обнаружения атак `snort` и писать сигнатуры для нее
...
Копируем файл из volume и открываем в UI `wireshark`:
...
- https://docs.snort.org/rules/options/non_payload/flags

### Вывод

В ходе выполнения данной лабораторной работы были изучены основные принципы работы сетевых утилит `nmap` и `snort`.

Были изучены основные методы определения ОС сервера при помощи `nmap` и созданы правила для `snort` для обнаружения
попыток определения ОС сервера.